...
path_in                       = '/scratch2/dicerbo/plot_test/prova/'
...
def plot_def(directory):
    ...
    k = 0
    for name in filedef:
        print '\tPlotting ' + name + ' file'
        plt.plotfile(path_out+name, delimiter = '\t', cols=(1, 2), comments='#', color = cdef[k], marker='.', mfc = cdef[k], mec = cdef[k], label = 'Log10P = '+str(pdef[k]), newfig=False)
        k += 1

    lgd = plt.legend(bbox_to_anchor=(1.75, 0.5), loc=5, borderaxespad=1.)
