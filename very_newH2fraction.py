...
def MolecularProfileTc():
    ...
    Density = 0.05; #in the middle of SF MUPPI cloud in phase diagram
    T_a = 2500.
    if not os.path.exists(path_out+'t'+str(T_a)):
    ...
    for p in lst:
        ...
        if press*mu_c*PROTONMASS/T_a < 10.**Dens.min() or press*mu_c*PROTONMASS/T_a > 10.**Dens.max():
            ...
            fp.flush()
            index += 1
            #line = '#Pressure %g at temperature %g is out of bound'%(p, T_a)
            #fp.write(line)
            continue
        else:
            Fcoll = molecular_fraction(press)
    ...
    while t < time:
        if ma <= 0. or press*mu_c*PROTONMASS/Temp < dmin or press*mu_c*PROTONMASS/Temp > dmax:
            if ma <= 0.:
                if index == 10:
                    if filectrl <= 3.:
                        line = '%e\t%e\t%e\n%e\t%e\t%e\n%e\t%e\t%e\n'%(0,np.log10(Rho_a), np.log10(Temp), 4e3,np.log10(Rho_a), np.log10(Temp), 8e3,np.log10(Rho_a), np.log10(Temp))
                        wr.write(line); filectrl = 0.
                    wr.flush(); wr.close()
                    t_atomic = Temp
                    dens_a = Rho_a
                    mmol = mass*FracC
